...
class DataLoaderNew(data.Dataset):
    ...
    def __init__(self, opt, image_list):
        ...
        coco_dict_path = os.path.join(opt.image_folder_root, 'data/coco_dict.json')
        coco_dict = json.load(open(coco_dict_path))
        # separate out indexes for each of the provided splits
        self.split_ix = {'new': []}
        for image in image_list:
            self.split_ix["new"].append(coco_dict[image])
        self.iterators = {'new': 0}
