...
def train(opt):
    ...
    acc_steps = getattr(opt, 'acc_steps', 1)  # accumulation steps
    ...
    if opt.start_from is not None:
        ...
        print("histories:\n ", histories)
        print("=" * 10)
        # x_nouse_justarubbish = input("read histories and go on?")
    # start_from为空，则将各信息量初始化
    else:
        infos['iter'] = 0
        infos['epoch'] = 0
    ...
    model = models.setup(opt).cuda()  # setup函数见__init__
    del opt.vocab
    ...
    def save_checkpoint(model, infos, optimizer, histories=None, append=''):
        ...
        # opt.checkpoint_path = log/log_aoanet
